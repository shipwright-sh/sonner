...
const Toast = (props: ToastProps) => {
	...
	createEffect(() => {
		...
		if (mergedProps.expanded || mergedProps.interacting || isDocumentHidden()) {
			pauseTimer();
		} else {
			startTimer();
		}
	...
	return (
		...
		>
			<Show
				when={
					closeButton() && !mergedProps.toast.jsx && toastType() !== "loading"
				}
			>
				<button
			...
			<Show when={mergedProps.toast.cancel} fallback={null}>
				{(() => {
			...
			<Show when={mergedProps.toast.action} fallback={null}>
				{(() => {
...
export const Toaster = (props: ToasterProps) => {
	...
	const hotkeyLabel = mergedProps.hotkey
		.join("+")
		.replace(/Key/g, "")
		.replace(/Digit/g, "");
