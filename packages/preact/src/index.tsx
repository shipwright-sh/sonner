...
const Toast = (props: ToastProps) => {
	...
	const swipeOutDirection = signal<"left" | "right" | "up" | "down" | null>(
		null,
	);
	...
	useEffect(() => {
		...
		if (toastNode) {
			...
			return () =>
				setHeights((h) =>
					h.filter((height) => height.toastId !== toastProp.id),
				);
		}
	...
	}, [expanded, interacting, toastProp, toastType, isDocumentHidden.value]);
	...
	return (
		<li
			...
			data-styled={!(toastProp.jsx || toastProp.unstyled || unstyled)}
...
export const Toaster = (props: ToasterProps) => {
	...
	const removeToast = (toastToRemove: ToastT) => {
		if (
			!toasts.value.find((toast: ToastT) => toast.id === toastToRemove.id)
				?.delete
		) {
			ToastState.dismiss(toastToRemove.id);
		}
		toasts.value = toasts.value.filter(
			({ id }: { id: string | number }) => id !== toastToRemove.id,
		);
	};
